...
const cloud=imageToCloud({data:pixels,width:2,height:2} as ImageData,{maxSplats:12,depthStrength:1,alphaThreshold:8},{landmarks,mask:new Float32Array([0,1,0,1]),maskWidth:2,maskHeight:2});
...
describe('pose-guided Gaussian pipeline',()=>{
  ...
  it('never exceeds the requested splat budget',()=>{const limited=imageToCloud({data:pixels,width:2,height:2} as ImageData,{maxSplats:4,depthStrength:1,alphaThreshold:8},{landmarks,mask:new Float32Array([1,1,1,1]),maskWidth:2,maskHeight:2});expect(limited.count).toBeLessThanOrEqual(4);});
  it('rejects an incomplete skeleton instead of using a cylindrical fallback',()=>{expect(()=>imageToCloud({data:pixels,width:2,height:2} as ImageData,{maxSplats:12,depthStrength:1,alphaThreshold:8},{landmarks:landmarks.slice(0,10),mask:new Float32Array([1,1,1,1]),maskWidth:2,maskHeight:2})).toThrow('人体骨格が不完全');});
