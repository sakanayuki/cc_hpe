...
import { BodyViewer, EDITABLE_BONES } from './body-viewer';
    ...
    <section class="workspace">
      <aside class="panel controls" aria-label="生成設定">
        ...
        <div id="poseControls" class="pose-controls hidden">
          ...
          <label class="field"><span>体幅 <output id="bodyWidthOut">100%</output></span><input id="bodyWidth" type="range" min="70" max="140" value="100"/></label>
          <label class="joint-field"><span>補正する関節</span><select id="joint">${EDITABLE_BONES.map(name=>`<option value="${name}">${name.replace('mixamorig:','')}</option>`).join('')}</select></label>
      ...
      <section class="viewer-card" aria-label="3Dビューアー">
        <div class="toolbar"><div><b id="stageLabel">STEP 1 · 素体GLBプレビュー</b><div class="segmented hidden" id="modes" role="group" aria-label="表示情報"><button class="active" data-mode="color">カラー</button><button data-mode="depth">深度</button><button data-mode="opacity">不透明度</button><button data-mode="confidence">信頼度</button></div></div><button id="reset" class="icon-button" title="視点をリセット">↻</button></div>
...
const bodyViewer = new BodyViewer(byId('viewer'));
...
byId('reset').addEventListener('click', () => bodyViewer.isVisible()?bodyViewer.reset():viewer.reset());
...
const poseDepth=byId<HTMLInputElement>('poseDepth'),bodyWidth=byId<HTMLInputElement>('bodyWidth'),joint=byId<HTMLSelectElement>('joint');
const jointAxes=['X','Y','Z'] as const;
poseDepth.addEventListener('input',()=>{byId<HTMLOutputElement>('poseDepthOut').value=`${poseDepth.value}%`;bodyViewer.setDepthScale(Number(poseDepth.value)/100);});
bodyWidth.addEventListener('input',()=>{byId<HTMLOutputElement>('bodyWidthOut').value=`${bodyWidth.value}%`;bodyViewer.setBodyWidth(Number(bodyWidth.value)/100);});
const applyJoint=()=>{const values=jointAxes.map(axis=>Number(byId<HTMLInputElement>(`joint${axis}`).value));bodyViewer.setJointCorrection(joint.value,values[0],values[1],values[2]);};
jointAxes.forEach(axis=>byId<HTMLInputElement>(`joint${axis}`).addEventListener('input',event=>{byId<HTMLOutputElement>(`joint${axis}Out`).value=`${(event.target as HTMLInputElement).value}°`;applyJoint();}));
joint.addEventListener('change',()=>{jointAxes.forEach(axis=>{byId<HTMLInputElement>(`joint${axis}`).value='0';byId<HTMLOutputElement>(`joint${axis}Out`).value='0°';});});
...
async function loadFile(file: File): Promise<void> {
  ...
  try { await image.decode(); sourceImage=image;pose=undefined;byId<HTMLImageElement>('thumb').src=url;byId('thumbWrap').classList.remove('hidden');byId('poseControls').classList.add('hidden');byId('exports').classList.add('hidden');estimate.disabled=false;generate.disabled=true;status.textContent=`${image.naturalWidth} × ${image.naturalHeight} px · STEP 1を実行してください`; }
  catch { URL.revokeObjectURL(url); status.textContent='画像を読み込めませんでした'; }
}

function clear(): void { sourceImage=undefined;pose=undefined;fileInput.value='';byId('thumbWrap').classList.add('hidden');byId('poseControls').classList.add('hidden');byId('exports').classList.add('hidden');estimate.disabled=true;generate.disabled=true;status.textContent='写真を選択してください'; }

estimate.addEventListener('click',async()=>{
  if(!sourceImage)return;estimate.disabled=true;status.textContent='人物の3D姿勢を推定し、GLB素体へ適用しています…';
  try {
    pose=await detectSinglePerson(sourceImage);await bodyViewer.showPose(pose,Number(poseDepth.value)/100);bodyViewer.setVisible(true);viewer.setVisible(false);byId('empty').classList.add('hidden');byId('poseControls').classList.remove('hidden');byId('stageLabel').textContent='STEP 1 · 姿勢付き素体GLB';byId('modes').classList.add('hidden');generate.disabled=false;status.textContent='素体を回転・補正して姿勢を確認し、STEP 2へ進んでください';
  }catch(error){pose=undefined;status.textContent=error instanceof Error?error.message:'姿勢推定に失敗しました';}finally{estimate.disabled=false;}
});

generate.addEventListener('click', async () => {
  if (!sourceImage||!pose) return; generate.disabled=true;status.textContent='確認した骨格と人物領域から3DGS化しています…'; await new Promise(requestAnimationFrame);
  try {
    pose.rigDepth=bodyViewer.captureDepth();pose.rigDepthWidth=512;pose.rigDepthHeight=512;
    ...
    viewer.setCloud(cloud);bodyViewer.setVisible(false);viewer.setVisible(true);byId('stageLabel').textContent='STEP 2 · 3D Gaussian Splat';byId('modes').classList.remove('hidden');byId('exports').classList.remove('hidden');byId('stats').textContent=`${cloud.count.toLocaleString('ja-JP')} splats`;status.textContent='3DGS化が完了しました';
