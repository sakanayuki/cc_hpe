...
const referenceImage = byId<HTMLImageElement>("referenceImage"),
  showReference = byId<HTMLInputElement>("showReference"),
  referenceOpacity = byId<HTMLInputElement>("referenceOpacity");
let referenceAngle = 0;
let stepOneActive = false;
const updateReference = () => {
  const requested = Number(referenceOpacity.value) / 100;
  const angleFade =
    referenceAngle <= 12 ? 1 : Math.max(0.12, 1 - (referenceAngle - 12) / 35);
  referenceImage.style.opacity = String(requested * angleFade);
  referenceImage.classList.toggle(
    "hidden",
    !stepOneActive || !showReference.checked || !referenceImage.src,
  );
};
...
const bodyViewer = new BodyViewer(byId("viewer"), selectJoint, (angle) => {
  ...
  warning.firstChild!.textContent = "正面以外では画像比較できません · ";
  warning.classList.toggle("hidden", angle <= 25 || !stepOneActive);
...
const viewer = new SplatViewer(
  ...
  (angle) => {
    const warning = byId("angle");
    warning.firstChild!.textContent = "品質保証範囲外 · ";
