...
`,
      fragmentShader: `precision highp float;uniform int mode;varying vec2 vUv;varying vec4 vColor;varying float vDepth,vConfidence,vLayer,vVisible;void main(){if(vVisible<.5)discard;float alpha=exp(-.5*dot(vUv,vUv)*9.)*vColor.a;if(alpha<.004)discard;vec3 c=vColor.rgb;if(mode==1)c=mix(vec3(.16,.08,.48),vec3(.18,1.,.72),clamp(vDepth*3.+.5,0.,1.));else if(mode==2)c=vec3(vColor.a);else if(mode==3)c=mix(vec3(1.,.12,.22),vec3(.2,1.,.55),vConfidence);else if(mode==4)c=vLayer<.5?vec3(.2,1.,.65):vLayer<1.5?vec3(.7,.48,1.):vec3(1.,.25,.5);gl_FragColor=vec4(c,alpha);}`,
    });
